...
class SygusGraph:
    ...
    def add_func(self, name):
        ...
        node, _ = self.add_node(SygusGraph.NODE_EMBEDDING_ARRAY["link_func"][funcid])
    ...
    def add_ast_literal(self, val):
        if val is True:
            node, _ = self.add_node(SygusGraph.NODE_EMBEDDING_ARRAY["link_literal_bool"][1])
        elif val is False:
            node, _ = self.add_node(SygusGraph.NODE_EMBEDDING_ARRAY["link_literal_bool"][0])
        else:
            node, _ = self.add_node()
            if val >= -20 and val < 20:
                self.set_node(
                    node, SygusGraph.NODE_EMBEDDING_ARRAY["link_literal_int_small"][val + 20]
                )
            else:
                self.set_node(
                    node,
                    SygusGraph.NODE_EMBEDDING_ARRAY["link_literal_int_large"][
                        1 if val > 0 else 0
                    ],
                )
...
class SygusSolver:
    ...
    def check_synth(self):
        ...
        for rule in synth.rules:
            ...
            self.graph.non_terminal_list[node_root] = rule.name
